...
### ~~CI's "Secret scan" job broken org-wide~~ — **done 2026-09-18**
Pushing the round-2 commits (once GitHub App access was fixed) surfaced this on PR #10: `gitleaks-action` refuses to run at all for an organization-owned repo without a `GITLEAKS_LICENSE` secret — `[DMForge-org] is an organization. License key is required.` Free for orgs, not a purchase — flagged as a PR comment rather than routed around. You signed up and added the secret, then asked for a retry — it failed identically, because the secret alone wasn't the whole fix: `.github/workflows/ci.yml`'s gitleaks step never referenced `GITLEAKS_LICENSE` in its `env:` block, only `GITHUB_TOKEN` (`1dddc03`). Added the missing line, pushed, reran — **CI is fully green (8/8) and PR #10's `mergeable_state` is `clean`.** Still a draft; flipping it to ready for review is your call.
